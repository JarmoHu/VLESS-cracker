...
typedef struct {
    ...
    int use_fingerprint_filter;
    ProbeStatus expected_fingerprint[MAX_PROBE_BYTES];
    size_t expected_fingerprint_count;

    // 新增控制参数
    int has_c_flag;
    unsigned int required_confirmation_rounds;
...
static void usage(const char *argv0) {
    fprintf(stderr,
            ...
            "       [-P <probe-file>] [-e <expected-sequence>] [-c <rounds>]\n"
            "Default probe file: " DEFAULT_PROBE_FILE "\n"
            "\n"
            "Examples:\n"
            "  sudo %s -i eth0\n"
            "  sudo %s -i eth0 -P my_probes.txt -e \"TO,ALERT,ALERT,ALERT\"\n"
            "  sudo %s -i eth0 -e \"TO,ALERT\" -c 3\n",
...
static int parse_expected_fingerprint(const char *value, ProbeStatus *fp, size_t *count, size_t cap) {
    *count = 0;
    const char *p = value;
    while (*p) {
        ...
        ProbeStatus status;
        size_t len = end - p;
        if (len == 2 && strncasecmp(p, "TO", 2) == 0) status = PROBE_STATUS_TIMEOUT;
        else if (len == 5 && strncasecmp(p, "ALERT", 5) == 0) status = PROBE_STATUS_ALERT;
        else if (len == 3 && strncasecmp(p, "FIN", 3) == 0) status = PROBE_STATUS_FIN;
        else if (len == 3 && strncasecmp(p, "RST", 3) == 0) status = PROBE_STATUS_RST;
        else if (len == 4 && strncasecmp(p, "NONE", 4) == 0) status = PROBE_STATUS_NONE;
        else {
            fprintf(stderr, "Unknown probe status '%.*s' in expected sequence.\n", (int)len, p);
            return 0;
        }

        if (*count >= cap) {
            fprintf(stderr, "Expected sequence exceeds maximum allowed probes (%zu).\n", cap);
            return 0;
        }
        fp[(*count)++] = status;
        p = end;
    ...
    for (int i = 0; i < MAX_PROBED_SERVERS; i++) {
        if (server_address_equal(&app->probed_servers[i], server_key)) {
            ...
            if (server->excluded || server->in_progress ||
                server->matched_rounds >= app->required_confirmation_rounds) {
                pthread_mutex_unlock(&app->lock);
                return 0;
            }
    ...
    *final_match_out = 0;
    ...
    for (int i = 0; i < MAX_PROBED_SERVERS; i++) {
        ...
        if (matched) {
            ...
            *matched_rounds_out = server->matched_rounds;
            if (server->matched_rounds >= app->required_confirmation_rounds) {
                *final_match_out = 1;
                server->excluded = 1;
            }
        } else {
...
// 核心调整点：实现支持提早返回、前缀匹配或严格匹配的告警判断逻辑
static int replay_probe_matches_alarm_condition(App *app,
    ...
    for (size_t i = 0; i < probe_count; i++) {
        if (replay_result_probe_status(&a[i]) != replay_result_probe_status(&c[i])) {
            mismatch_found = 1;
    ...
    // 如果没有使用 -e 的预期指纹，则任何一个 mismatch 都视为触发了警报条件
    if (!app->use_fingerprint_filter) {
        return 1;
    }

    if (app->has_c_flag) {
        // 提供了 -c 标志，说明要求“这几轮是完整的”，因此我们进行完整的严格匹配
        if (probe_count != app->expected_fingerprint_count) {
            return 0;
        }
        for (size_t i = 0; i < probe_count; i++) {
            if (replay_result_probe_status(&a[i]) != app->expected_fingerprint[i]) {
                return 0;
            }
        }
        return 1;
        ...
        // 没有提供 -c 标志但提供了 -e 指纹
        // “当第一个mismatch，且A的序列和 -e中的序列前面相同就行了，不用等它全部计算完”
        if (first_mismatch_idx >= app->expected_fingerprint_count) {
            return 0; 
        }
        for (size_t i = 0; i <= first_mismatch_idx; i++) {
            if (replay_result_probe_status(&a[i]) != app->expected_fingerprint[i]) {
                return 0;
            }
        }
...
static void print_replay_comparison(App *app, 
                                    const FlowKey *client_key,
    ...
    int alarm_condition = replay_probe_matches_alarm_condition(app, a, c, probe_count);
    ...
    if (!mismatch) {
        ...
        printf("\033[31m[%s.%06ld] TLS ReplayProbe MISMATCH, VLESS/REALITY connection detected after %u/%u confirmations. client=%s:%u server=%s:%u sni=%s probes=%zu %s result=%s",
               timebuf, (long)ts.tv_usec,
               app->required_confirmation_rounds,
               app->required_confirmation_rounds,
...
static void *replay_probe_worker(void *arg) {
    ...
    if (a_status != PROBE_ROUND_VALID ||
        ...
        int round_matches = replay_probe_matches_alarm_condition(task->app, a, c,
                                                                 probe_count);
        finish_server_probe_round(task->app, &task->server_key,
                                  round_matches, 0,
                                  &matched_rounds, &final_match);
        if (final_match) {
            print_replay_comparison(task->app, &task->client_key, &task->server_key,
            ...
            log_infof("TLS replay confirmation round=%u/%u passed server=%s:%u\n",
                      matched_rounds, task->app->required_confirmation_rounds,
...
int main(int argc, char **argv) {
    ...
    // 默认配置
    app->has_c_flag = 0;
    app->required_confirmation_rounds = 1; // 如果没有 -c，默认为 1（第一轮错就短路触发）

    while ((opt = getopt(argc, argv, "i:f:l:d:p:P:e:c:h")) != -1) {
        switch (opt) {
        case 'i':
            iface = optarg;
            break;
        ...
        case 'p':
        {
            ...
            if (!add_probe_spec(&probes, &probe_count, &probe_cap, bytes, len)) {
        ...
        case 'P':
            ...
            break;
        case 'e':
            if (!parse_expected_fingerprint(optarg, app->expected_fingerprint, &app->expected_fingerprint_count, MAX_PROBE_BYTES)) {
                usage(argv[0]);
                free(probes);
                free(app);
                return 1;
            }
            app->use_fingerprint_filter = 1;
            break;
        case 'c':
            if (!parse_uint_arg(optarg, 1, 100, &app->required_confirmation_rounds)) {
                fprintf(stderr, "invalid confirmation rounds: %s\n", optarg);
                usage(argv[0]);
                free(app);
                return 1;
            }
